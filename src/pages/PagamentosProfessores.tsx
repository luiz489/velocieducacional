...
export default function PagamentosProfessores() {
  ...
  const gerar = useMutation({
    ...
    onSuccess: (data) => {
      ...
      qc.invalidateQueries({ queryKey: ["pagamentos-professores"] });
    },
  ...
  const semDadosBancarios = (pagamentos ?? []).filter(
    (p) => !p.banco_codigo && !p.chave_pix
  );
  ...
  const exportarCSV = () => {
    const linhas = (pagamentos ?? []).filter(
      (p) => selecionados.size === 0 || selecionados.has(p.conta_id)
    );
    if (linhas.length === 0) {
    ...
    const cabecalho = [
      "Favorecido", "CNPJ", "Banco", "Codigo_Banco", "Agencia", "Conta",
      "Tipo_Conta", "Chave_PIX", "Valor", "Data_Vencimento",
    ];
    const linhasCSV = linhas.map((p) => [
      p.razao_social || p.professor_nome,
      p.cnpj || "",
      p.banco_nome || "",
      p.banco_codigo || "",
      p.agencia || "",
    ...
    a.href = url;
    a.download = `pagamentos_professores_${mes}_${ano}.csv`;
    a.click();
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Banknote className="h-6 w-6" /> Pagamentos de Professores (PJ)
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Gere as contas a pagar do mês e exporte os dados bancários para enviar à instituição financeira.
          </p>
        </div>
        <Button onClick={() => gerar.mutate()} disabled={gerar.isPending}>
          <RefreshCw className="h-4 w-4 mr-2" />
          {gerar.isPending ? "Gerando..." : "Gerar Pagamentos do Mês"}
        </Button>
      </div>
      ...
      {semDadosBancarios.length > 0 && (
        <div className="flex items-start gap-3 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4">
          ...
          <p className="text-sm text-amber-700 dark:text-amber-400">
            {semDadosBancarios.length} professor(es) sem dados bancários ou chave PIX cadastrados —
            complete o cadastro deles na tela de Professores antes de exportar para o banco.
          </p>
        </div>
      )}
                ...
                {(!pagamentos || pagamentos.length === 0) && (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                      ...
                      "Gerar Pagamentos do Mês" para criar as contas a pagar dos professores PJ ativos.
                    </TableCell>
                  </TableRow>
                )}
                {pagamentos?.map((p) => (
                  <TableRow key={p.conta_id}>
                    ...
                    <TableCell className="font-medium">
                      {p.professor_nome}
                      {p.razao_social && <div className="text-xs text-muted-foreground">{p.razao_social}</div>}
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">{p.cnpj || "—"}</TableCell>
