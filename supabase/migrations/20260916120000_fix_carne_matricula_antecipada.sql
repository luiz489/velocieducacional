-- Bug: gerar_parcelas_para_matricula montava o carnê a partir do MÊS da
-- data_ingresso, sem olhar o ano. Numa matrícula antecipada (assinada num
-- ano anterior ao ano letivo da turma - comum: setembro/outubro pra
-- temporada seguinte), isso deslocava as 12 mensalidades inteiras (ex.:
-- matrícula em 09/2026 pra turma do ano letivo 2027 gerava carnê rodando
-- 09/2027 a 08/2028 em vez de 01/2027 a 12/2027). Só não dava problema por
-- coincidência quando a matrícula antecipada caía em janeiro (mês 1).
--
-- Fix: se a matrícula for antecipada (ano da data_ingresso < ano_letivo da
-- turma), o carnê sempre começa em janeiro do ano letivo - não no mês da
-- assinatura. Matrícula no mesmo ano do ano letivo (inclusive fora de
-- janeiro, ex. transferência em março) continua começando no mês real de
-- ingresso, sem mudança de comportamento.
--
-- Corrigidos manualmente antes desta migration: 11 matrículas já geradas
-- erradas na DM Núcleo (Benjamim, Bento e outros 9 alunos com matrícula
-- antecipada de 09/2026 pro ano letivo 2027).
