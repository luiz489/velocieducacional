-- Taxa de matricula: regra de pontualidade + arredondamento (lote 3)
--
-- 1. gerar_parcelas_para_matricula: as parcelas da Taxa de Matricula agora gravam
--    valor_integral (valor cheio, sem desconto, com opcionais), pra seguirem a
--    mesma regra das mensalidades - perder o desconto se pagas apos o vencimento.
--    Alem disso a ultima parcela da taxa absorve o residuo do arredondamento
--    (ex.: 1000/3 nao deixa mais 999,99).
-- 2. trg_financeiro_aplica_regra_pontualidade: guard NEW.valor > 0 pra uma
--    parcela de bolsa 100% (R$ 0,00) nunca ser inflada pro valor cheio.
...
BEGIN
  ...
  -- Taxa de Matricula = valor de UMA mensalidade (base + opcionais), ja com o
  -- desconto. valor_integral = o mesmo valor sem o desconto, pra regra de
  -- pontualidade valer tambem na taxa.
...
BEGIN
  IF NEW.status = 'Pago'
     ...
     AND NEW.valor_integral IS NOT NULL
     AND NEW.valor > 0
