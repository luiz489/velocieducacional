-- Achado testando a correção de dia de vencimento negociado numa matrícula
-- que já tinha mensalidade paga: recalcular_financeiro_matricula apaga só
-- as parcelas Pendente/Atrasado (preserva as pagas, corretamente), mas
-- gerar_parcelas_para_matricula sempre reinseria TODAS as N mensalidades
-- do zero (i = 1..numero_parcelas), sem checar se aquele número de parcela
-- já estava quitado - resultado: "Mensalidade 1/12" duplicada (uma Paga,
-- outra Pendente reaparecendo no mesmo mês), toda vez que se recalcula uma
-- matrícula com pelo menos uma parcela já paga.
--
-- Fix: antes de inserir cada mensalidade/parcela da taxa, pula se já existe
-- uma linha com aquela mesma descrição pra essa matrícula que NÃO está
-- Pendente/Atrasado (ou seja, já foi Paga ou Cancelada) - o mesmo critério
-- que recalcular_financeiro_matricula já usa pra decidir o que preservar.
